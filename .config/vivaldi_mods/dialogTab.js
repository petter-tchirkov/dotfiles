...
(() => {
  ...
  class DialogMod {
      ...
      if (webviewData) {
        ...
        tabEl.addEventListener(
          ...
          () => {
            ...
            // Restore focus to the original page.
            requestAnimationFrame(() => {
              const activeWebview = document.querySelector(
                ".active.visible.webpageview webview",
              );
              if (activeWebview && typeof activeWebview.focus === "function") {
                activeWebview.focus();
              }
            });
          },
    ...
    showDialog(linkUrl, fromPanel) {
      ...
      if (activeWebview) {
        ...
        dialogTab.style.width = rect.width * 0.8 + "px";
      ...
      // Focus the dialog content (webview) so keyboard works immediately.
      requestAnimationFrame(() => {
  ...
  class WebsiteLinkInteractionHandler {
    ...
    /**
     * Checks if a link is clicked by the middle mouse while pressing Ctrl + Shift, then fires an event with the Url
     */
    #initialize() {
    ...
    #setupMouseHandling() {
      ...
      const interceptAltClick = (event) => {
        if (!(event.ctrlKey && event.shiftKey) || ![0, 1].includes(event.button)) return;
        const linkInfo = this.#getLinkElement(event);
        if (!linkInfo) return;

        // Stop Chromium's default click handling on links
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();

        if (!this.dialogTriggered) {
          this.#callDialog(event, linkInfo);
        }
      };

      document.addEventListener("click", interceptAltClick, true);
      document.addEventListener("auxclick", interceptAltClick, true);

      document.addEventListener("pointerdown", (event) => {
        // Check if the Ctrl key, Shift key, and mouse button were pressed
        if (event.ctrlKey && event.shiftKey && [0, 1].includes(event.button)) {
    ...
    #setupIconHandling() {
      ...
      document.addEventListener(
        ...
        this.debounce((event) => {
          ...
          requestAnimationFrame(() => {
            const rect = linkInfo.element.getBoundingClientRect();
          ...
          this.icon.dataset.targetUrl = linkInfo.url;

          linkInfo.element.addEventListener(
            "mouseleave",
            this.#hideLinkIcon.bind(this),
          );
    ...
    #getLinkElement(event) {
      ...
      const urlPattern =
        /^(https?:|file:|ftp:|mailto:|chrome:|vivaldi:|about:|\/\/|\/|\.\/|\.\.\/)/i;
      const isUrlCandidate = (value) => {
      ...
      const preferredCandidates = [
        typeof element.href === "string" ? element.href : element.getAttribute("href"),
      ...
      let rawUrl =
        preferredCandidates.find((val) => isUrlCandidate(val)) || "";

      if (!rawUrl) {
        for (const attr of element.attributes) {
          if (attr.name === "style") continue; // avoid matching background URLs
          if (isUrlCandidate(attr.value)) {
            rawUrl = attr.value;
            break;
          }
        }
      ...
      try {
        return { element, url: new URL(rawUrl, location.href).href };
      } catch (e) {
        return { element, url: rawUrl.trim() };
      }
    }
    ...
    #callDialog(event, linkInfo) {
      let link = linkInfo || this.#getLinkElement(event);
      if (link) {
        event.preventDefault();
        event.stopPropagation();
        this.dialogTriggered = true;
        this.#sendDialogMessage(link.url);
      }
    }
